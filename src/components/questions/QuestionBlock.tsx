                ...
                `}>
                    <div className="col-span-8 lg:col-span-6">
                        ...
                        <div className="space-x-2">
                            {
                                tags.filter(
                                    tag => question.tags.includes(tag.id) || (question.tags as (number|string)[]).includes(tag.text)
                                ).map(tag => (
                                    <span key={tag.id} className={`${tag.colorScheme?.text} ${tag.colorScheme?.bg} ${tag.colorScheme?.ring} text-xs ring-1 py-0.5 px-2 rounded-xl`}>
                                        {tag.text}
                                    </span>
                                ))
                            }
