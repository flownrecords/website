  ...
  return (
    <LayerGroup>
      {sigmets.map(sig => {
            ...
            const from = new Date(sig.validTimeFrom * 1000).toLocaleString("en-GB", { 
                timeZone: "UTC", day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" 
            }).replace(/:|,/g, '') + 'Z'; 
            const to   = new Date(sig.validTimeTo * 1000).toLocaleString("en-GB", { 
                timeZone: "UTC", day: "2-digit", month: "2-digit", year: "numeric", hour: "2-digit", minute: "2-digit" 
            }).replace(/:|,/g, '') + 'Z';
            ...
            if(displaySigmets) {
                return (
                    ...
                    positions={latlngs}
                    pathOptions={{
                        color: colorCoding[sig.hazard as keyof typeof colorCoding] || "gray",
                        fillOpacity: 0.1,
                        weight: 1,
                        dashArray: "3,3",
                    }}
                        ...
                        <Popup className="w-[300px]">
                            <div className="">
                                ...
                                <div className="flex-col gap-1 flex">
                                    <span><span className="font-semibold">Hazard</span> {sig.hazard}</span>
                                    <span><span className="font-semibold">Qualifier</span> {sig.qualifier}</span>
                                    <span><span className="font-semibold">Top</span> {top}</span>
                                    <span><span className="font-semibold">Base</span> {base}</span>
                                    <span><span className="font-semibold">Valid From</span> {from}</span>
                                    <span><span className="font-semibold">Valid To</span> {to}</span>
